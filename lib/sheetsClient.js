...
async function clearSheetFormatting(sheets, spreadsheetId, sheetId) {
  // Полный сброс листа перед тем, как заново построить статическую
  // разметку. Важно сначала снять все склейки (unmergeCells) — иначе
  // новая раскладка с другими размерами блоков конфликтует со старыми
  // merge-регионами, оставшимися от прошлых версий (например, из
  // Apps Script), и Sheets API отклоняет запрос на merge с ошибкой
...
async function applyValueUpdates(sheets, spreadsheetId, data) {
  ...
  await sheets.spreadsheets.values.batchUpdate({
    spreadsheetId,
    // USER_ENTERED (а не RAW) — чтобы формулы вида =IMAGE(...) реально
    // рендерились как картинки, а не оставались текстом. Обычные числа
    // и строки при этом интерпретируются точно так же, как раньше.
