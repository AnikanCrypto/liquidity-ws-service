...
// Bitget помечает токенизированные акции префиксом "R" перед тикером
// (rNVDAUSDT, rSPYUSDT...) — определяем и по возможности исключаем их
// из обычного крипто-топа, иначе они забивают весь список (у них там
// объёмы в миллиарды — Bitget сам рекламирует "$1B+ tokenized stock volume").
function isTokenizedStockSymbol(symbol, quoteAsset) {
  if (!symbol.startsWith('R')) return false;
  const base = symbol.slice(1, symbol.length - quoteAsset.length).toUpperCase();
  return TRADEFI_BASE_TICKERS.includes(base);
}
...
async function fetchBitget() {
  ...
  const spot = spotList
    .filter(d => d.symbol.endsWith(QUOTE_ASSET) && !isTokenizedStockSymbol(d.symbol, QUOTE_ASSET))
