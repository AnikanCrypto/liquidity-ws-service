...
const { normalizeBase } = require('./comparison');
...
// USDC/USDT — своп двух стейблкоинов, а не показатель спроса на монету.
// Исключаем такие пары из топа по объёму на всех биржах.
const EXCLUDED_BASE_TICKERS = new Set(['USDC']);

function isExcludedStable(symbol, exchangeName) {
  return EXCLUDED_BASE_TICKERS.has(normalizeBase(symbol, exchangeName));
}
...
// а также NVDA/MU/золото-серебро/полупроводниковые ETF, которые
// обнаружились в фьючерсах Binance/Bybit/OKX под обычным USDT-тикером.
const TRADEFI_BASE_TICKERS = [
  'AAPL', 'MSFT', 'NVDA', 'TSLA', 'AMZN', 'GOOGL', 'GOOG', 'META', 'NFLX',
  'COIN', 'HOOD', 'MSTR', 'AMD', 'AVGO', 'INTC', 'BA', 'JPM', 'MU',
  'XAG', 'WTI', 'BRENT', 'XTI', 'XBR', // золото (XAU) сознательно не исключаем — оставляем в топах
  'SPX', 'US500', 'NAS100', 'NDX', 'US30', 'DJI',
  'JP225', 'N225', 'KR200',
  'QQQ', 'SPY', 'VOO', 'IVV', 'SOXL', 'SOXS', 'SOXX', 'PLTR', 'MRVL',
  'SPCX', 'SNDK', 'CRCL', 'TSM', 'SKHYNIX', 'SKHY', 'KORU', 'SNXX',
];

// Извлечь базовый тикер из символа "NVDAUSDT" -> "NVDA"
// (формат Binance/Bybit — тикер и котируемая валюта слитно).
function plainBase(symbol, quoteAsset) {
  return symbol.endsWith(quoteAsset) ? symbol.slice(0, symbol.length - quoteAsset.length).toUpperCase() : symbol.toUpperCase();
}

// То же самое для формата OKX: "SNDK-USDT-SWAP" / "BTC-USDT" -> часть до первого дефиса.
function okxBase(instId) {
...
async function fetchWhiteBit() {
  ...
  const spot = markets
    .filter(m => m.type === 'spot' && m.money === QUOTE_ASSET && !isExcludedStable(m.name, 'WhiteBIT'))
    .map(m => ({ symbol: m.name, volume: volumeOf(m.name) }));

  const futuresAll = markets.filter(m => m.type === 'futures' && m.money === QUOTE_ASSET && !isExcludedStable(m.name, 'WhiteBIT'));
  const cryptoFutures = [];
  const tradeFi = [];
...
async function fetchBybit() {
  ...
  const spot = spotList
    .filter(d => d.symbol.endsWith(QUOTE_ASSET) && !TRADEFI_BASE_TICKERS.includes(plainBase(d.symbol, QUOTE_ASSET)) && !isExcludedStable(d.symbol, 'Bybit'))
    .map(d => ({ symbol: d.symbol, volume: parseFloat(d.turnover24h) }));

  // Акции/сырьё у Bybit встречаются именно среди фьючерсов — делим этот
  // список на обычные крипто-фьючи и TradeFi, а не просто выкидываем.
  const futures = [];
  const tradeFi = [];
  linearList.forEach(d => {
    if (!d.symbol.endsWith(QUOTE_ASSET) || isExcludedStable(d.symbol, 'Bybit')) return;
    const item = { symbol: d.symbol, volume: parseFloat(d.turnover24h) };
    (TRADEFI_BASE_TICKERS.includes(plainBase(d.symbol, QUOTE_ASSET)) ? tradeFi : futures).push(item);
  });

  return { Spot: topByVolume(spot, TOP_N), Futures: topByVolume(futures, TOP_N), TradeFi: topByVolume(tradeFi, TOP_N) };
}
...
async function fetchOkx() {
  ...
  const spot = spotList
    .filter(d => d.instId.endsWith('-' + QUOTE_ASSET) && !TRADEFI_BASE_TICKERS.includes(okxBase(d.instId)) && !isExcludedStable(d.instId, 'OKX'))
  ...
  // (та же поправка, что и в Apps Script версии). Акции/сырьё у OKX
  // тоже сидят именно в SWAP-списке — делим его на крипто и TradeFi,
  // а не просто выкидываем (у OKX это даже отдельная вкладка "TradFi").
  const futures = [];
  const tradeFi = [];
  swapList.forEach(d => {
    if (!d.instId.endsWith('-' + QUOTE_ASSET + '-SWAP') || isExcludedStable(d.instId, 'OKX')) return;
    const item = { symbol: d.instId, volume: parseFloat(d.volCcy24h) * parseFloat(d.last) };
    (TRADEFI_BASE_TICKERS.includes(okxBase(d.instId)) ? tradeFi : futures).push(item);
  });
...
async function fetchBitget() {
  ...
  const spot = spotList
    .filter(d => d.symbol.endsWith(QUOTE_ASSET) && !isBitgetTokenizedStock(d.symbol, QUOTE_ASSET) && !isExcludedStable(d.symbol, 'Bitget'))
...
async function fetchKuCoin() {
  ...
  const spot = spotList
    .filter(d => d.symbol.endsWith('-' + QUOTE_ASSET) && !isExcludedStable(d.symbol, 'KuCoin') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.symbol, 'KuCoin')))
    .map(d => ({ symbol: d.symbol, volume: parseFloat(d.volValue) }));
  const futures = futuresList
    .filter(d => d.quoteCurrency === QUOTE_ASSET && !isExcludedStable(d.symbol, 'KuCoin') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.symbol, 'KuCoin')))
...
async function fetchMexc() {
  ...
  const spot = spotData
    .filter(d => d.symbol.endsWith(QUOTE_ASSET) && !isExcludedStable(d.symbol, 'MEXC') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.symbol, 'MEXC')))
    .map(d => ({ symbol: d.symbol, volume: parseFloat(d.quoteVolume) }));
  const futures = futuresList
    .filter(d => d.symbol.endsWith('_' + QUOTE_ASSET) && !isExcludedStable(d.symbol, 'MEXC') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.symbol, 'MEXC')))
...
async function fetchGate() {
  ...
  const spot = spotData
    .filter(d => d.currency_pair.endsWith('_' + QUOTE_ASSET) && !isExcludedStable(d.currency_pair, 'Gate.io') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.currency_pair, 'Gate.io')))
  ...
  const futures = futuresData
    .filter(d => !isExcludedStable(d.contract, 'Gate.io') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.contract, 'Gate.io')))
    .map(d => ({ symbol: d.contract, volume: parseFloat(d.volume_24h_quote) }));
...
async function fetchKraken() {
  ...
  const futures = futuresList
    .filter(d => d.tag === 'perpetual' && d.pair && (d.pair.endsWith(':USD') || d.pair.endsWith(':USDT')) && !isExcludedStable(d.symbol, 'Kraken') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.symbol, 'Kraken')))
    .map(d => ({ symbol: d.symbol, volume: parseFloat(d.volumeQuote) }));

  return {
    Spot: topByVolume(spotItems.filter(i => !isExcludedStable(i.symbol, 'Kraken') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(i.symbol, 'Kraken'))), TOP_N),
    Futures: topByVolume(futures, TOP_N),
  };
}
...
async function fetchHtx() {
  ...
  const spot = spotList
    .filter(d => d.symbol.endsWith(QUOTE_ASSET.toLowerCase()) && !isExcludedStable(d.symbol, 'HTX') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.symbol, 'HTX')))
    .map(d => ({ symbol: d.symbol.toUpperCase(), volume: parseFloat(d.vol) }));
  const futures = futuresList
    .filter(d => d.contract_code.endsWith('-' + QUOTE_ASSET) && !isExcludedStable(d.contract_code, 'HTX') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d.contract_code, 'HTX')))
...
async function fetchBitfinex() {
  ...
  const spot = data
    .filter(d => typeof d[0] === 'string' && d[0].charAt(0) === 't' && d[0].endsWith('UST') && d[0].indexOf(':') === -1 && !isExcludedStable(d[0], 'Bitfinex') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d[0], 'Bitfinex')))
    .map(d => ({ symbol: d[0], volume: parseFloat(d[8]) * parseFloat(d[7]) }));
  const futures = data
    .filter(d => typeof d[0] === 'string' && d[0].charAt(0) === 't' && d[0].indexOf(':USTF0') !== -1 && !isExcludedStable(d[0], 'Bitfinex') && !TRADEFI_BASE_TICKERS.includes(normalizeBase(d[0], 'Bitfinex')))
    .map(d => ({ symbol: d[0], volume: parseFloat(d[8]) * parseFloat(d[7]) }));
...
async function fetchBinanceRanking() {
  ...
  const spot = spotData
    .filter(d => d.symbol.endsWith(QUOTE_ASSET) && !TRADEFI_BASE_TICKERS.includes(plainBase(d.symbol, QUOTE_ASSET)) && !isExcludedStable(d.symbol, 'Binance'))
    .map(d => ({ symbol: d.symbol, volume: parseFloat(d.quoteVolume) }));
  const futures = futuresData
    .filter(d => d.symbol.endsWith(QUOTE_ASSET) && !TRADEFI_BASE_TICKERS.includes(plainBase(d.symbol, QUOTE_ASSET)) && !isExcludedStable(d.symbol, 'Binance'))
    .map(d => ({ symbol: d.symbol, volume: parseFloat(d.quoteVolume) }));
...
// Акции/сырьё у Binance тоже сидят во фьючерсах (NVDAUSDT, XAUUSDT и
// т.п.) — TradeFi не нуждается в реальном времени (как и у остальных
// бирж), поэтому тянем обычным REST-поллингом отдельно от WS-механики
// крипто-топа выше.
async function fetchBinanceTradeFi() {
  const futuresData = await getJson('https://fapi.binance.com/fapi/v1/ticker/24hr');
  const tradeFi = futuresData
    .filter(d => d.symbol.endsWith(QUOTE_ASSET) && TRADEFI_BASE_TICKERS.includes(plainBase(d.symbol, QUOTE_ASSET)))
    .map(d => ({ symbol: d.symbol, volume: parseFloat(d.quoteVolume) }));

  return { TradeFi: topByVolume(tradeFi, TOP_N) };
}
