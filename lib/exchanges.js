...
async function fetchOkx() {
  ...
  const spot = spotList
    .filter(d => d.instId.endsWith('-' + QUOTE_ASSET) && !TRADEFI_BASE_TICKERS.includes(okxBase(d.instId)))
  ...
  const futures = swapList
    .filter(d => d.instId.endsWith('-' + QUOTE_ASSET + '-SWAP') && !TRADEFI_BASE_TICKERS.includes(okxBase(d.instId)))
