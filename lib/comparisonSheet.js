...
const STATUS_COLORS = {
  good: '#E3F2E4',      // совпадает / вне топа для capped-монеты — всё ок
  raise: '#FFF3CD',      // нужно поднять выше
  lower: '#FDE9D9',      // нужно опустить ниже
  add: '#FCE8E6',       // нет в топ-20 WhiteBIT — добавить
  neutral: '#F1F3F4',    // есть на WhiteBIT, но не сравнивается (нет в топе у других)
  pinned: '#E0E7FF',     // нативный токен — особый случай, не трогать
  violation: '#F28B82',  // нарушение бизнес-правила (capped-монета залезла в топ)
};
...
// Одна таблица (Spot+Futures друг под другом) на весь лист — вызывается
// дважды с разными sheetId для двух отдельных листов
// ("Сравнение-3-биржи" и "Сравнение-10-бирж").

const TOTAL_COLS = 9; // # | Монета | Binance | Bybit | OKX | Бирж | Ø ранг | Ранг WhiteBIT | Статус
...
function buildStaticRequests(sheetId, layout, titleText, headerLabels) {
  ...
  // на случай, если лист остался уже, чем нужно, с прошлых версий.
  requests.push({
    updateSheetProperties: {
      properties: { sheetId, gridProperties: { columnCount: TOTAL_COLS + 5, rowCount: layout.lastRow + 10 } },
  ...
  const widths = [40, 100, 80, 80, 80, 60, 70, 100, 380];
  widths.forEach((w, i) => {
...
// rowsByType: { Spot: [alignment rows...], Futures: [...] }
...
// Подсветка всей строки по смыслу (colorKey из comparison.js) —
// пересчитывается каждый цикл, т.к. значения (а значит и цвет) меняются.
function buildStatusColorRequests(sheetId, layout, rowsByType) {
  const requests = [];
  ['Spot', 'Futures'].forEach(mt => {
    const t = layout[mt];
    const rows = rowsByType[mt] || [];
    for (let i = 0; i < TOP_ROWS; i++) {
      ...
      const bg = r ? (STATUS_COLORS[r.colorKey] || undefined) : undefined;
      requests.push({
        repeatCell: {
          range: { sheetId, startRowIndex: rowIdx - 1, endRowIndex: rowIdx, startColumnIndex: 0, endColumnIndex: TOTAL_COLS },
          cell: { userEnteredFormat: { backgroundColor: bg ? rgb(bg) : { red: 1, green: 1, blue: 1 } } },
...
module.exports = { buildLayout, buildStaticRequests, buildValueUpdates, buildStatusColorRequests, STATUS_COLORS };
