...
const { iconFormula } = require('./icons');

// ==================== НАСТРОЙКИ ====================

const TOP_N = 20;
const COLS_PER_EXCHANGE = 4; // Ранг | Иконка | Символ | Объём
const GAP_COLS = 1;
...
function buildStaticRequests(sheetId, layout) {
  ...
  // иначе все последующие merge/format-запросы на колонки за пределами
  // текущей сетки листа отклоняются с ошибкой "exceeds grid limits".
  // Число колонок могло вырасти между деплоями (например, при добавлении
  // колонки под иконку), а старый лист остаётся той же ширины, что была
  // при его создании — Sheets сам не расширяется.
  requests.push({
  ...
  layout.exchanges.forEach(ex => {
    ...
    ex.types.forEach(t => {
      ...
      // Мини-шапка колонок: Ранг | (иконка, без подписи) | Символ | Объём
      requests.push({
        updateCells: {
          range: {
            sheetId,
      ...
      formatRange(t.dataStartRow, t.dataEndRow, ex.colStart, ex.colStart, { horizontalAlignment: 'CENTER' }); // Ранг
      formatRange(t.dataStartRow, t.dataEndRow, ex.colStart + 1, ex.colStart + 1, { horizontalAlignment: 'CENTER' }); // Иконка
      formatRange(t.dataStartRow, t.dataEndRow, ex.colStart + 3, ex.colStart + 3, { // Объём (теперь 4-я колонка)
      ...
      // Подсветка топ-3 — трогаем только backgroundColor, чтобы не стирать
      // номер-формат/выравнивание, выставленные чуть выше.
      [1, 2, 3].forEach(r => {
    ...
    // Ширины колонок биржи: Ранг | Иконка | Символ | Объём
    const widths = [40, 24, 140, 130];
    widths.forEach((w, i) => {
      requests.push({
        updateDimensionProperties: {
          range: { sheetId, dimension: 'COLUMNS', startIndex: ex.colStart - 1 + i, endIndex: ex.colStart + i },
  ...
  layout.exchanges.forEach(ex => {
    ex.types.forEach(t => {
      ...
      for (let r = 0; r < TOP_N; r++) {
        ...
        if (item) {
          const base = normalizeBase(item.symbol, ex.name);
          rows.push([r + 1, iconFormula(base), item.symbol, item.volume]);
        } else {
          rows.push(['', '', '', '']);
        }
      }
