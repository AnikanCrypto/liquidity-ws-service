...
function buildStaticRequests(sheetId, layout) {
  ...
  function mergeAndFormat(rowStart, colStart, numCols, { bg, fg, bold, fontSize, align }) {
    const range = {
      sheetId,
  ...
  function formatRange(rowStart, rowEnd, colStart, colEnd, format) {
    requests.push({
      repeatCell: {
        range: {
          sheetId,
  ...
  layout.exchanges.forEach(ex => {
    ...
    mergeAndFormat(ex.headerRow, ex.colStart, COLS_PER_EXCHANGE, {
      bg: ex.headerColor, fg: '#FFFFFF', bold: true, fontSize: 12,
    });
    ...
    ex.types.forEach(t => {
      ...
      [1, 2, 3].forEach(r => {
        ...
        if (rowIdx <= t.dataEndRow) {
          formatRange(rowIdx, rowIdx, ex.colStart, ex.colStart + 2, { backgroundColor: rgb(RANK_COLORS[r]) });
        }
      });
    });
...
function buildValueUpdates(sheetName, layout, state, timestampLabel) {
  ...
  layout.exchanges.forEach(ex => {
    ...
    ex.types.forEach(t => {
      ...
      const label = isError ? `${t.type} — ошибка: ${items.__error}` : `${t.type} — топ ${count} по объёму`;
